  ...
  useEffect(() => {
    ...
    const fetchProdutos = async () => {
      try {
        ...
        const data = await get("/ares", token); // Faz a requisição GET
  ...
  return (
    <div className="">
      ...
      <div className="flex justify-center items-center space-x-4 bg-[#ffffffe3] py-5">
        <Wind size={35} />
        <h1 className="text-3xl">Dados sobre o ar - {local}</h1>
      ...
      <section className="ml-20 flex  justify-evenly pt-6 ">
        ...
        {dadosApi && (
          <CardPages
            title="Indice de qualidade do ar"
            icon={<Wind size={32} />}
            bgColor="bg-[rgba(255,255,255,0.69)] hover:rgba(255,255,255,1.0)] transition-all duration-500"
            dado1={dadosApi}
            descricao="Qualidade dos ares gravada nos últimos meses"
            descricao2={`Ultima qualidade do ar:${dadosApi[0]?.tipo}: ${dadosApi[0]?.dado}`}
